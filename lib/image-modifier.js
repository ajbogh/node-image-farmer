...
var imageModifier = {
    ...
    modifyImageProcess: function(processOptions, options, appConfig){
        return new Promise(function(resolve, reject){
            ...
            console.log('node', __dirname+'/image-modifier.js', '--processOptions', env.processOptions, '--options', env.options, '--appConfig', env.appConfig);
            ...
            child.on('exit', function (exitCode, signal) {
                if(!resolved){
                    console.log("--Not resolved, aborting child", signal);
                    reject(exitCode);
                }
                console.log("Child exited with code: " + exitCode, signal);
            });
        });
    },
